...
#if 0
/*
 * Find ELF load address
 * XXX: Doesn't work for PIE
 */
static void *get_elf_base(const char *filename)
{
	...
	ssize_t ret;
	Elf64_Ehdr ehead;
	ret = fread(&ehead, sizeof(ehead), 1, fp);
	if (ret != 1) {
		perror("fread()");
		goto err_close;
	}
...
int main(int argc, char *argv[])
{
	...
	fprintf(stderr, "[+] Initialization done! Entering trace loop\n");
